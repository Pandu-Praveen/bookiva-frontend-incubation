import{A as o}from"./config-XSboSa-d.js";feather.replace();let h,b,g;var u,v=!1;const L=async(i,s)=>{const t=u.filter(l=>l.hallName===i),e=[];t[0].reservations&&t[0].reservations.forEach(l=>e.push(l.reservedOn.split("-").reverse().join("-")));let a=[];const r={altInput:!0,minDate:"today",maxDate:new Date().fp_incr(15),inline:!0,dateFormat:"d-m-Y",onChange:(l,f,j)=>{a=[],console.log(f),v=l.length>0,f.includes(",")?(a=f.split(", "),a=a.map(c=>c.split("-").reverse().join("-").trim())):a.push(f.split("-").reverse().join("-").trim());const p=document.getElementById("login-btn");if(s){async function c(){await fetch(o+"/managementprebook",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({EMAIL:y,HALL:h,SEATS:g,DATES:a}),credentials:"include"}).then(m=>{m.status===200&&p.setAttribute("href","/managementbook/")})}c()}else{b=a[0];async function c(){await fetch(o+"/prebook",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({EMAIL:y,HALL:h,SEATS:g,DATE:b}),credentials:"include"}).then(m=>{m.status===200&&p.setAttribute("href","/book/")})}c()}t[0].isAvailable&&v?p.classList.remove("disabled"):p.classList.add("disabled")}};s?(r.mode="multiple",delete r.maxDate):r.disable=e,flatpickr("#calendar",r)};window.addEventListener("DOMContentLoaded",async()=>{try{u=await(await fetch(o+"/venues",{credentials:"include"})).json(),$(u)}catch(i){console.error("Error fetching venue data:",i)}});function $(i){document.querySelector(".cards-container"),i.forEach(s=>{w+=`<div class="hall-card">
            <div class="hall-img">
            ...
            <a href="#" class="butt-main details-btn ff-inter fs-s" data-venue="${s.hallName}">DETAILS</a>
        </div>
`}),N.innerHTML=`<div class="overlay overlay-hidden"></div>
    <div class="popup-modal-wrapper popup-hidden"></div>`+w,document.querySelectorAll(".details-btn").forEach(s=>{s.addEventListener("click",t=>{t.preventDefault(),e();async function e(){await fetch(o+"/profile",{credentials:"include",headers:{"Content-Type":"application/json"}}).then(a=>{a.status===401&&(location.href="/login/")})}A(t.target.dataset.venue)})})}const E=document.querySelector(".user-menu"),N=document.querySelector(".cards-container");document.querySelector(".user-nav").addEventListener("click",i=>{E.classList.toggle("user-menu-hidden")});var w="",d,k;function A(i){d=document.querySelector(".popup-modal-wrapper"),k=document.querySelector(".overlay"),screen.width<=500&&document.body.classList.toggle("body-noscroll"),k.classList.toggle("overlay-hidden"),d.classList.toggle("popup-hidden"),d.innerHTML="",d.classList.contains("popup-hidden")||T(i)}let n,S,y;C();async function C(){if(n=await(await fetch(o+"/profile",{credentials:"include",headers:{"Content-Type":"application/json"}})).json(),S=n.name,y=n.email,n.blockStatus){location.href="/block/";return}else if(n){let s;n.role=="MANAGEMENT"?s='<a class="ff-inter fs-2s user-menu-link" href="/management/">Profile</a>':n.role=="USER"?s='<a class="ff-inter fs-2s user-menu-link" href="/profile/">Profile</a>':n.role=="ADMIN"?location.href="/admin/":location.href="/login/";const t=document.querySelector(".user-menu");t.innerHTML=`
        <h3 class="ff-inter">Welcome, ${S}</h3>
        ${s}
        ...
        `,document.querySelector(".log-out").addEventListener("click",async e=>{const a=await fetch(o+"/logout",{credentials:"include",headers:{"Content-Type":"application/json"}}),r=await a.json();console.log(a,r),a.status==200&&(location.href="/login/")})}}function T(i){v=!1;const t=u.filter(e=>e.hallName===i).map(e=>(h=e.hallName,g=e.seatingCapacity,`
...
`)}
                    ...
                    <div class="popup-details">
                        ...
                        <a id="login-btn" href="#" class="ff-inter butt-main sign-in fs-s ${e.isAvailable&&v?"":"disabled"}" >Book Now</a>
                    </div>
